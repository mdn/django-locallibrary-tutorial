...
@permission_required('catalog.can_mark_returned')
def assign_memorizables_to_mp_locations_and_numbers_week_target_fixed_category(request, pk):
    """View function for assigning a week_target to a memory palace location and number."""
    #Separate MP:
...
def assign_memorizables_to_mp_locations_and_numbers_workpackage_relevantinformation_tobememorized_fixed_category(request, pk):
    """View function for assigning a workpackage_relevantinformation to a memory palace location and number for a fixed category (e.g. Job, Private)."""

    #Week targets for headlines in html:
    ...
    #Workpackage relevant information to be memorized:
    memorizable_objects_tobeassignedto_mp_locations__workpackage_relevantinformation_tobememorized = BookInstance.objects.filter(book__author__t_memorization_package_mp_technique_assignmenttype_category=pk).order_by('memorization_sequence')

    # If this is a POST request then process the Form data
    if request.method == 'POST':

        # Create a form instance and populate it with data from the request (binding):
        form = Assign_Memorizables_To_MP_Locations_And_Numbers_Workpackage_Relevantinformation_Tobememorized_Form(request.POST)
        ...
        if form.is_valid():
            ...
            assigned_memory_palace_fromform_first = form.cleaned_data['assigned_memory_palace_first']
            assigned_memory_palace_fromform_second = form.cleaned_data['assigned_memory_palace_second']
            assigned_memory_palace_fromform_third = form.cleaned_data['assigned_memory_palace_third']
            assigned_memory_palace_fromform_fourth = form.cleaned_data['assigned_memory_palace_fourth']
            assigned_memory_palace_fromform_fifth = form.cleaned_data['assigned_memory_palace_fifth']
            maximum_capacity_assigned_memory_palace_fromform_first = assigned_memory_palace_fromform_first.number_of_memorypalace_datapoints_perlocation        #.annotate(abc = Max('t_memory_palace_type_location_number__memory_palace_number'))
            maximum_capacity_assigned_memory_palace_fromform_second = assigned_memory_palace_fromform_second.number_of_memorypalace_datapoints_perlocation
            maximum_capacity_assigned_memory_palace_fromform_third = assigned_memory_palace_fromform_third.number_of_memorypalace_datapoints_perlocation
            maximum_capacity_assigned_memory_palace_fromform_fourth = assigned_memory_palace_fromform_fourth.number_of_memorypalace_datapoints_perlocation
            maximum_capacity_assigned_memory_palace_fromform_fifth = assigned_memory_palace_fromform_fifth.number_of_memorypalace_datapoints_perlocation
            
            #Initialize and start for-loop:
#            i = 1
            for mem_ri in memorizable_objects_tobeassignedto_mp_locations__workpackage_relevantinformation_tobememorized:
                #1st MP
                if mem_ri.memorization_sequence <= maximum_capacity_assigned_memory_palace_fromform_first:
                    mem_ri.t_memory_palace_type_location_id = assigned_memory_palace_fromform_first.id
                    mem_ri.t_memory_palace_type_location_number_id = T_Memory_Palace_Type_Location_Number.objects.get(t_memory_palace_type_location_id = assigned_memory_palace_fromform_first.id, memory_palace_number = mem_ri.memorization_sequence).id
                    mem_ri.save()
#                    i+=1
                #2nd MP
                elif mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first <= maximum_capacity_assigned_memory_palace_fromform_second:
                    mem_ri.t_memory_palace_type_location_id = assigned_memory_palace_fromform_second
                    #Fehlermeldung für leichteres Fixing:
#                    if i - maximum_capacity_assigned_memory_palace_fromform_first <= 0:
#                        print('No dense rank in t_workpackage_relevantinformation_tobememorized.memorization_sequence')
                    try:
                        mem_ri.t_memory_palace_type_location_number_id = T_Memory_Palace_Type_Location_Number.objects.get(t_memory_palace_type_location_id = assigned_memory_palace_fromform_second.id, memory_palace_number = mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first).id
                    except T_Memory_Palace_Type_Location_Number.DoesNotExist as inst:
                        print(inst)
                        print('i - maximum_capacity_assigned_memory_palace_fromform_first =', i - maximum_capacity_assigned_memory_palace_fromform_first)
                        print('Possibly no dense rank in t_workpackage_relevantinformation_tobememorized.memorization_sequence')
                        raise
                    mem_ri.save()
#                    i+=1
                #3rd MP
                elif mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second <= maximum_capacity_assigned_memory_palace_fromform_third:
                    mem_ri.t_memory_palace_type_location_id = assigned_memory_palace_fromform_third
                    try:
                        mem_ri.t_memory_palace_type_location_number_id = T_Memory_Palace_Type_Location_Number.objects.get(t_memory_palace_type_location_id = assigned_memory_palace_fromform_third.id, memory_palace_number = mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second).id
                    except T_Memory_Palace_Type_Location_Number.DoesNotExist as inst:
                        raise
                    mem_ri.save()
#                    i+=1
                #4th MP
                elif mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second - maximum_capacity_assigned_memory_palace_fromform_third <= maximum_capacity_assigned_memory_palace_fromform_fourth:
                    mem_ri.t_memory_palace_type_location_id = assigned_memory_palace_fromform_fourth
                    try:
                        mem_ri.t_memory_palace_type_location_number_id = T_Memory_Palace_Type_Location_Number.objects.get(t_memory_palace_type_location_id = assigned_memory_palace_fromform_fourth.id, memory_palace_number = mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second - maximum_capacity_assigned_memory_palace_fromform_third).id
                    except T_Memory_Palace_Type_Location_Number.DoesNotExist as inst:
                        raise
                    mem_ri.save()
#                    i+=1
                #5th MP
                elif mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second - maximum_capacity_assigned_memory_palace_fromform_third - maximum_capacity_assigned_memory_palace_fromform_fourth <= maximum_capacity_assigned_memory_palace_fromform_fifth:
                    mem_ri.t_memory_palace_type_location_id = assigned_memory_palace_fromform_third
                    try:
                        mem_ri.t_memory_palace_type_location_number_id = T_Memory_Palace_Type_Location_Number.objects.get(t_memory_palace_type_location_id = assigned_memory_palace_fromform_fifth.id, memory_palace_number = mem_ri.memorization_sequence - maximum_capacity_assigned_memory_palace_fromform_first - maximum_capacity_assigned_memory_palace_fromform_second - maximum_capacity_assigned_memory_palace_fromform_third - maximum_capacity_assigned_memory_palace_fromform_fourth).id
                    except T_Memory_Palace_Type_Location_Number.DoesNotExist as inst:
                        raise
                    mem_ri.save()
#                    i+=1
            ...
            # redirect to a new URL:
            return HttpResponseRedirect(reverse('assign-memorizables-to-mp-locations-and-numbers-workpackage-relevantinformation-tobememorized-fixed-category', kwargs={'pk': pk}))
    # If this is a GET (or any other method) create the default form
    else:
        assigned_memory_palace_proposed_first = T_Memory_Palace_Type_Location.objects.filter(t_memory_palace_type=1).annotate(number_of_memorypalace_datapoints_perlocation = Count('t_memory_palace_type_location_number', distinct=True)).annotate(lastusage_date = Max('t_memory_palace_type_location_packageassignment_timeseries__assignment_to_memorization_package_datetime')).order_by('lastusage_date')[0]
        assigned_memory_palace_proposed_second = T_Memory_Palace_Type_Location.objects.filter(t_memory_palace_type=1).annotate(number_of_memorypalace_datapoints_perlocation = Count('t_memory_palace_type_location_number', distinct=True)).annotate(lastusage_date = Max('t_memory_palace_type_location_packageassignment_timeseries__assignment_to_memorization_package_datetime')).order_by('lastusage_date')[1]
        assigned_memory_palace_proposed_third = T_Memory_Palace_Type_Location.objects.filter(t_memory_palace_type=1).annotate(number_of_memorypalace_datapoints_perlocation = Count('t_memory_palace_type_location_number', distinct=True)).annotate(lastusage_date = Max('t_memory_palace_type_location_packageassignment_timeseries__assignment_to_memorization_package_datetime')).order_by('lastusage_date')[2]
        assigned_memory_palace_proposed_fourth = T_Memory_Palace_Type_Location.objects.filter(t_memory_palace_type=1).annotate(number_of_memorypalace_datapoints_perlocation = Count('t_memory_palace_type_location_number', distinct=True)).annotate(lastusage_date = Max('t_memory_palace_type_location_packageassignment_timeseries__assignment_to_memorization_package_datetime')).order_by('lastusage_date')[3]
        assigned_memory_palace_proposed_fifth = T_Memory_Palace_Type_Location.objects.filter(t_memory_palace_type=1).annotate(number_of_memorypalace_datapoints_perlocation = Count('t_memory_palace_type_location_number', distinct=True)).annotate(lastusage_date = Max('t_memory_palace_type_location_packageassignment_timeseries__assignment_to_memorization_package_datetime')).order_by('lastusage_date')[4]

        form = Assign_Memorizables_To_MP_Locations_And_Numbers_Workpackage_Relevantinformation_Tobememorized_Form(initial={'assigned_memory_palace_first': assigned_memory_palace_proposed_first,'assigned_memory_palace_second': assigned_memory_palace_proposed_second,'assigned_memory_palace_third': assigned_memory_palace_proposed_third,'assigned_memory_palace_fourth': assigned_memory_palace_proposed_fourth,'assigned_memory_palace_fifth': assigned_memory_palace_proposed_fifth})
